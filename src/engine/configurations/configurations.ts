// This module contains handles to the extension's configurations.
...
/** 
 * Handle to the `leaper.decorateAll` configuration.
 * 
 * The value of this configuration determines whether all pairs or just the ones nearest to each 
 * cursor are decorated. 
 */
export const decorateAll = new VCDualReader({
...
/**
 * Handle to the `leaper.decorationOptions` configuration.
 * 
 * The value of this configuration determines the style of the decorations applied.
 * 
 ...
 */
export const decorationOptions = new VCDualReader({
    ...
    //      object without a `rangeBehavior` property or `null` (see the next point for why we allow 
    //      `null`), because the actual `DecorationRenderOptions` type has too many properties to 
    //      manually typecheck. New properties are always being added to that type by vscode, so it 
    //      is difficult for us to keep up with it. 
    //
    //   2. If the user wants to disable decorations, we require them to specify `null` instead of 
    //      an empty object because of a bug in vscode (as of version 1.58.0) where if a configuration 
    //      is set to `{}` in a workspace or workspace folder that previously did not have a value 
    //      for that configuration, the `{}` will be ignored.
    //
    ...
    transform: (v: Object | null): Unchecked<DecorationRenderOptions> => {
         ...
         *
         * For color properties, users are allowed to specify either a hex [RGB(A) value] string or 
         * a [theme color identifier] string. However, the vscode API requires a `ThemeColor` object 
         * when theme colors are specified. Thus, we have to go through the configuration value and 
         * convert every property that specifies a theme color identifer to a `ThemeColor`.
         * 
        ...
        function convertColors(obj: Object): void {
            for (const key of Reflect.ownKeys(obj)) {
                ...
                if (typeof value === 'object' && value !== null) {
                    ...
                    // If a string specifying a color starts with a `#`, we treat it as a hex RGB(A) 
                    // value and leave it as is. Otherwise, we treat it as a theme color identifier.
    ...
    // The behavior of the deprecated configuration is to pass the object specified by the user as 
    // is to vscode. Thus, as long as the object is non-null, we accept it.
    // 
    // Note the null check is needed because `null` is an object in JS.
    deprValidate: (v: unknown): v is Object => typeof v === 'object' && v !== null,

    // To preserve behavior of the deprecated configuration, we do not convert the colors like we do 
    // for the new configuration. 
    //
    // Overall, we maintain the behavior we have always had, which is to keep every property except 
...
/**
 * Handle to the `leaper.detectedPairs` configuration.
 * 
 * The value of this configuration determines which pairs are detected and then tracked. 
 * 
 * OPTIMIZATION NOTE: It might be tempting to replace the type of this configuration with a `Set`, 
 * but micro benchmarks (https://jsbench.me/5qkqg886lo/1) show that for small array sizes (such as 
 * the default value of this configuration), checking for inclusion is faster with an array than it 
 * is with a set.
 */
export const detectedPairs = new VCDualReader({

    name: 'leaper.detectedPairs',
    validate: (arr: unknown): arr is string[] => {
        return Array.isArray(arr) 
            && arr.length <= DETECTED_PAIRS_MAX_ITEMS
    ...
    deprTransform: (deprValue: { open: string, close: string }[]) => {
        ...
        // which pair was to be tracked. However, that configuration only allowed the user to specify 
        // _additional_ pairs to be tracked. There was a base set of pairs that could not be changed; 
        // This is that set.
...
/**
 * The maximum number of pairs that can be specified for the `leaper.detectedPairs` configuration.
 * 
 * We limit the number of pairs that can be specified for the `leaper.detectedPairs` configuration 
 * to prevent a malicious workspace from slowing down the extension by supplying a huge array for 
 * that configuration. With this limit in place, it should be safe to enable that configuration in 
 * untrusted workspaces.
 */
export const DETECTED_PAIRS_MAX_ITEMS = 100;
